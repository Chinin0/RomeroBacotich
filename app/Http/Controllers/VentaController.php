...
class VentaController extends Controller
{
    ...
    public function VentasById()
    {
        ...
        $ventas = Venta::where('idusuario', $idUsuario)->paginate(10);
    ...
    public function index()
    {
        ...
        for ($i = 0; $i < count($ventas); $i++) {
            $cliente = User::find($ventas[$i]['idusuario']);
            $ventas[$i]['idcliente'] = $cliente->name;
        }
    ...
    public function show($id)
    {
        ...
        return view('ventas.show', compact('venta', 'productos', 'usuario', 'detalles'));
    }
    ...
    {
        $user = Auth::user();
        $productosCarrito = \Cart::session($user->id)->getContent();

        // Crear una nueva venta
        $venta = Venta::create([
            'idusuario' => $user->id,
            'tipo_comprobante' => 'Reserva',
            'num_comprobante' => '000000',
            'fecha_hora' => now(),
            'impuesto' => 0,
            'total' => 0, // Se actualizará más adelante
            'estado' => 'Reservado',
        ...
        // Calcular el total de la reserva y crear los detalles de venta
        $totalVenta = 0;
        ...
        return redirect()->route('reservations.index')->with('success', 'Productos reservados exitosamente.');
    }

    public function payReservations(Request $request)
    {
        $user = auth()->user();
        $reservations = Venta::where('idusuario', $user->id)->where('estado', 'Reservado')->with('detalleVentas.producto')->get();

        $totalAmount = 0;
        foreach ($reservations as $reservation) {
            $totalAmount += $reservation->total;
        }
        return view('pagos.crear', ['order' => $reservations, 'total' => $totalAmount]);

        //Integrar con la pasarela de pago...
        foreach ($reservations as $reservation) {
    ...
    {
        $user = Auth::user();
        $reservations = Venta::where('idusuario', $user->id)
